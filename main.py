...
async def on_message(message: discord.Message):
    ...
    if not message.guild or not is_call(message.content):
        return
    ...
    # 送信者がVCにいる時だけ、同じVCで返信を読み上げる。
    voice = getattr(message.author, "voice", None)
    channel = getattr(voice, "channel", None)
    if isinstance(channel, discord.VoiceChannel):
        stay = STAY_VC.get(gid)
        # 常駐中は指定VCから動かさない。別VCからの呼びかけにはテキストだけ返す。
        if stay and channel.id != stay:
            return

        queued = await enqueue_audio(gid, channel.id, "tts_full", reply)
        if not queued:
